...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Only compile proto files on Linux (TPU is Linux-only) and only when
    // the `tpu` feature is enabled. The proto compilation requires `protoc`
    // to be installed, which is not desirable for consumers that only need
